...
### 다. 캐싱 및 데이터 무효화(Invalidation) 전략 부재
...
### 가. 기본(Default) 액션과 명명된(Named) 액션의 혼용 문제
...
### 나. 프로그레시브 인핸스먼트(`use:enhance`)의 오해 또는 미활용
...
### 가. CSRF(Cross-Site Request Forgery) 보호 미흡
...
### 나. XSS(Cross-Site Scripting) 방지 실패
...
### 라. Shallow Routing의 부적절한 사용 (SvelteKit 2+)
...
- `svelte-kit package` 명령어를 사용하거나 번들 분석 도구(예: `rollup-plugin-visualizer`)를 활용하여 번들 크기를 정기적으로 확인하고 최적화한다.
- 필요한 모듈만 임포트하고, 사용하지 않는 코드는 제거한다.
- 동적 임포트(`import()`)를 사용하여 필요할 때만 코드를 로드하는 코드 스플리팅(code splitting)을 적극적으로 활용한다.

### 라. 서버리스 함수 콜드 스타트(Cold Start) 문제 간과
...
### 라. `reroute` 훅의 과도한 사용 또는 비효율적 구현
